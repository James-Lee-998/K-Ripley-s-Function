...
for file in pbar(dir):
    try:
        DIR_HOME = str(file)
        PROTEIN_DF = pd.DataFrame(pd.read_csv(PATH + "\\" + DIR_HOME + '\\SQSTM1.txt' , sep = "\t", header = None, names = ['X','Y','Value']), columns = ['X','Y','Value'])
        ...
        img_SQSTM1 = Image.new(mode='RGB',size=(maxX, maxY))
        pixels = img_SQSTM1.load()
        PROTEIN_DF.apply(setColour_SENS, axis=1)
        img_SQSTM1_1 = img_SQSTM1.transpose(PIL.Image.FLIP_TOP_BOTTOM)
        img_SQSTM1_1.save(PATH + "\\" + DIR_HOME + "/HEATMAP_PROT_" + str(THRESHOLD) + ".png")
